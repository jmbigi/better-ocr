...
- **Descarga de música gratuita (`descarga_musica.py`):** CLI para descargar audio clásico gratuito y legal desde Internet Archive. Subcommands: `demo` (10 piezas preseleccionadas), `buscar "query"` (búsqueda libre), `listar-fuentes`. Fuentes verificadas en vivo 2026-09-05: Internet Archive Search API + metadata API funcionan sin autenticación; Musopen (403), Musiqpub (caído), IMSLP (sin API programática). Protecciones: rate limit 1 req/seg, limite 500 MB por archivo, solo MP3 (no Ogg/Flac), dry-run mode. Patrón idéntico a `rns.py` (stdlib puro, argparse, funciones testeables). Piezas demo: Goldberg Variations (Landowska), Partita No. 3 (Laredo), Brandenburg No. 1, Claro de Luna (Beethoven), Primavera (Vivaldi), Canon en D (Pachelbel), Gymnopedie No. 1 (Satie), Clair de Lune (Debussy).
