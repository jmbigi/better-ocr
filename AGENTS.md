...
- **Verificación de sintaxis:** `python3 -m py_compile extractor_final.py chart_server.py ocr_rapido.py vision.py captcha_ia.py captcha_web.py revision.py` (sin dependencias externas).
- **Pruebas unitarias:** `python3 -m unittest discover -s tests -v` (solo stdlib + pandas + openpyxl; sin paddleocr, usa modelos simulados). Ejecútalas al tocar `extractor_final.py`, `chart_server.py`, `ocr_rapido.py`, `vision.py`, `captcha_ia.py`, `captcha_web.py` o `revision.py`.
